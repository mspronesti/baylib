...
namespace bn {
    namespace inference{
        ...
        struct bcvec {
            ...
            bcvec(int dim, const compute::context& context, compute::command_queue &queue ,ushort cardinality): cardinality(cardinality){
                vec = compute::vector<int>(dim, context);
            }
        };

        /*
         * Contains simulate_node results in both reduced and bvec form.
         * Used by logic_sampling to access bvec of already computed nodes
         * Keeps count of how many times a bvec has been used, and deletes the reference after a bvec has been used out_degree times.
         * Can be added to compressed_result to cast it directly
         */
        template <typename Probability>
        class uncompressed_result {
        public:
            explicit uncompressed_result(bn::vertex<Probability> nedges):
            data(std::vector<std::pair<std::vector<ulong>, std::optional<bcvec>>>(nedges)),
            use_count(std::vector<ulong>(nedges,0))
            {}
            ...
            std::pair<std::vector<ulong>, std::optional<bcvec>> get(const bn::vertex<Probability> v) {
                ...
                BAYLIB_ASSERT(use_count[v] > 0,
                              "Function get has been called on node "+std::to_string(v)+" too many times",
                              std::runtime_error)
                              use_count[v]--;
                if (use_count[v] == 0)
                    data[v].second.reset();
        ...
        /*
         * Contains results in reduced form (number of realizations of a specific state)
        ...
        private:
            ...
            ulong nsamples;
            bn::vertex<Probability> nedges;
        public:
            //nsamples is not assigned from the constructor, it will get its value from uncompressed_results
            compressed_result(ulong ns, bn::vertex<Probability> ne) : nsamples(0), nedges(ne), data(std::vector<std::vector<ulong>>(ne) )
            {nsamples = ns;}
            ...
            compressed_result& operator+=(compressed_result &other) {
                BAYLIB_ASSERT(other.nsamples == nsamples,
                              ...
                              std::runtime_error)
                              BAYLIB_ASSERT(other.nedges == nedges,
                                            "Edge number does not agree.",
                                            std::runtime_error)

                                            for (bn::vertex<Probability> i = 0; i<nedges; i++) {
        ...
        template <typename Probability>
        class logic_sampling : abstract_inference_algorithm<Probability>{
        ...
        public:
            logic_sampling(
                    ...
                    const compute::device &device = compute::system::default_device()
                            ): abstract_inference_algorithm<Probability>(samples), seed(seed), context(compute::context(device)),
                            queue(compute::command_queue(context, device)), memory(memory){}

            std::pair<std::vector<ulong>, bcvec> simulate_node(std::vector<Probability> striped_cpt,
                                                               const std::vector<bcvec>& parents_result,
                                                               int dim,
                                                               int possible_states,
                                                               compute::default_random_engine rand_eng);
            bn::marginal_distribution<Probability> make_inference(const bn::bayesian_network<Probability> &bn) override;
        ...
        private:
            ...
            std::vector<Probability> accumulate_cpt(std::vector<Probability> striped_cpt, bn::state_t possible_states);
            std::vector<ulong> compute_result_general(bcvec &res);
            void calculate_iterations(const bayesian_network<Probability> &bn);
            bn::marginal_distribution<Probability> compute_total_result(const bayesian_network<Probability> &bn, compressed_result <Probability> &cr);

        };

        template<typename Probability>
        bn::marginal_distribution<Probability> logic_sampling<Probability>::make_inference(const bn::bayesian_network<Probability> &bn) {
            ...
            BAYLIB_ASSERT(std::all_of(var.begin(), var.end(),
                          ...
                          std::vector<bn::vertex<Probability>> vertex_queue = bn::sampling_order(bn);
        ...
        template <typename Probability>
        std::vector<Probability> logic_sampling<Probability>::accumulate_cpt(std::vector<Probability> striped_cpt, bn::state_t possible_states){
            for(bn::state_t i = 0 ; i < striped_cpt.size() ; i += possible_states)
        ...
        template <typename Probability>
        std::pair<std::vector<ulong>, bcvec> logic_sampling<Probability>::simulate_node(const std::vector<Probability> striped_cpt,
                                                                                        const std::vector<bcvec>& parents_result,
                                                                                        int dim,
                                                                                        int possible_states,
                                                                                        compute::default_random_engine rand_eng){
        ...
        void logic_sampling<Probability>::calculate_iterations(const bayesian_network<Probability> &bn) {
            ...
            if(sample_p < this->nsamples){
                ...
                this->niter = this->nsamples / sample_p;
            }
            else {
        ...
        template<typename Probability>
        bn::marginal_distribution<Probability> logic_sampling<Probability>::compute_total_result(const bayesian_network<Probability> &bn, compressed_result<Probability> &cr) {
            bn::marginal_distribution<Probability> total_result(bn.variables());
            for (bn::vertex<Probability> v = 0; v< cr.get_nedges();v++) {
