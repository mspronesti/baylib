...
template std::vector<uint> inf::logic_sampler(baylib::cuda_graph<float>& graph, const std::vector<ulong>& sim_order, uint samples, bool evidence);
template std::vector<uint> inf::logic_sampler(baylib::cuda_graph<double>& graph, const std::vector<ulong>& sim_order, uint samples, bool evidence);
template std::vector<float> inf::likelihood_weighting_sampler(baylib::cuda_graph<float>& graph, const std::vector<ulong>& sim_order, uint samples);
template std::vector<float> inf::likelihood_weighting_sampler(baylib::cuda_graph<double>& graph, const std::vector<ulong>& sim_order, uint samples);
...
namespace baylib {
    namespace inference {
        ...
        __global__ void
        simulate_logic_kernel(const baylib::cuda_variable<Probability> *graph,
            ...
            for (int iter = 0; iter < n_iter; iter++) {
                ...
                for (int i = 0; i < n_vars; i++) {
                    ...
                    if (check_evidence && graph[index].is_evidence && graph[index].evidence_state != var_result) {
                        valid_sample = false;
                        break;
                    }
        ...
        template<typename Probability>
        std::vector<uint>
        logic_sampler(baylib::cuda_graph<Probability> &graph, const std::vector<ulong> &sim_order, uint samples, bool evidence) {
        ...
        template<typename Probability>
        __global__ void
        simulate_likelihood_kernel(baylib::cuda_variable<Probability> *graph,
                                   const ushort *sim_order,
                                   uint n_vars,
            ...
            for (int i = 0; i < n_vars; i++) {
                ...
                if (n_parents == 0) {
                    ...
                    if (graph[index].is_evidence) {
                        var_result = graph[index].evidence_state;
        ...
        template<typename Probability>
        std::vector<float>
        likelihood_weighting_sampler(baylib::cuda_graph<Probability> &graph, const std::vector<ulong> &sim_order,
                                     uint samples) {
            ushort *sim_order_device;
            float *marginal_result_device;
            ushort *running_size_device;
            curandState *cu_rand_seeds;
            std::vector<ushort> sim_order_u(sim_order.begin(), sim_order.end());
            ...
            gpuErrcheck(cudaMalloc(&cu_rand_seeds, kp.N_Blocks * kp.N_Threads * sizeof(curandState)))
            gpuErrcheck(cudaMemset(marginal_result_device, 0, sizeof(float) * tot_states * kp.N_Blocks));

            baylib::setup_kernel<<<kp.N_Blocks, kp.N_Threads>>>(cu_rand_seeds);
            ...
            gpuErrcheck(cudaFree(sim_order_device));
            gpuErrcheck(cudaFree(cu_rand_seeds));
